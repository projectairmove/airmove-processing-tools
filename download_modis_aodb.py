...
@click.command()
@click.option('--drive-folder', default="MODIS_AODB")
@click.option('--local-folder', default="MODIS_AODB")
...
def download_modis_aodb(
    ...
    logger = get_logger(
        script_name="download_modis_aodb",
    ...
    _info(f"MODIS_AODB processing from: {from_date} to {to_date}.")
    ...
    def getDailyAve(n):
        ...
        sysIndexinfo = ee.String(f"MODIS_AODB_{from_date}_{to_date}")
    ...
    geetools.batch.Export.imagecollection.toDrive(
        ...
        namePattern='MODIS_AODB_{system_date}',
        folder=drive_folder,
        region=ncr_box,
        scale=1000,
        verbose=True
