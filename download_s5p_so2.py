...
@click.command()
@click.option('--drive-folder', default="S5P_SO2")
...
def download_s5p_so2(
    ...
    _info(f"S5P_SO2 processing from: {from_date} to {to_date}.")
    ...
    so2 = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_SO2').select('SO2_column_number_density').filterBounds(ncr_shape).filterDate(from_date, to_date)
    val_max = 0.0002
    val_min = 0
    band_viz = {
        "min": val_min,
    ...
    geetools.batch.Export.imagecollection.toDrive(
        collection=so2,
        namePattern="S5P_SO2_{system_date}",
        folder=drive_folder,
        region=ncr_box,
        verbose=True)
